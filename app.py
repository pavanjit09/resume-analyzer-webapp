...
rf_classifier_categorization = pickle.load(open('models/rf_categorization.pkl', 'rb'))
tfidf_vectorizer_categorization = pickle.load(open('models/tfidf_categorization.pkl', 'rb'))
rf_classifier_job_recommendation = pickle.load(open('models/rf_recommendation.pkl', 'rb'))
tfidf_vectorizer_job_recommendation = pickle.load(open('models/tfidf_jrecommendation.pkl', 'rb'))
...
def extract_education_from_resume(text):
    education = [] 
    degree_pattern = r"(?i)Degree:\s*.*"
    college_pattern = r"(?i)College:\s*.*"
    cgpa_pattern = r"(?i)CGPA:\s*.*"

    degree_match = re.search(degree_pattern, text)
    if degree_match:
        education.append(degree_match.group(0)) 

    college_match = re.search(college_pattern, text)
    if college_match:
        education.append(college_match.group(0)) 

    cgpa_match = re.search(cgpa_pattern, text)
    if cgpa_match:
        education.append(cgpa_match.group(0))  
